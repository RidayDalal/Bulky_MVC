...
namespace BulkyWeb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = CreateUser();
                // Populating the email of the user.
                await _userStore.SetUserNameAsync(user, Input.Email, CancellationToken.None);
                await _emailStore.SetEmailAsync(user, Input.Email, CancellationToken.None);
                // Populating the other entered properties of the user.
                ...
                // Checks if the user is successfully logged in, i.e., it checks if the user 
                // account has been successfully registered in the database.
                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");
                    // If the user has selected a specific role from the dropdown, assign 
                    // that particular role to the user.
                    if (!String.IsNullOrEmpty(Input.Role))
                    {
                        await _userManager.AddToRoleAsync(user, Input.Role);
                    // If the user has NOT chosen any specific role, then by default, assign them 
